...
* **term**: terms are any (unique) variables that appears in a regression model, like response variable, predictors or random effects. A "term" only relates to the unique occurence of a variable. For instance, in the expression `x + I(x^2)`, there is only the term `x`.
* **variables**: A variable is considered as an object that stores unique data information. For instance, the expression `x + I(x^2)` has two objects with two different data values, and thus are treated as two variables.
...
# find all variables, i.e. 
find_variables(model)
```

The reason for having functions that cover similar aspects of a model object (like `find_variables()` and `find_predictors()` or `find_terms()`)