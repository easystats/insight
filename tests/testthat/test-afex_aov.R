if (require("testthat") && require("insight") && require("afex")) {
  ...
  test_that("afex_aov: formula and parameters", {
    ...
    expect_equal(find_variables(Mc2),
                 list(response = "value",
                      fixed = c("treatment", "gender", "age", "phase", "hour"),
                      random = "id"))
    expect_equal(find_predictors(Mc2, effects = "all"),
                 list(fixed = c("treatment", "gender", "age", "phase", "hour"),
                      random = "id"))
    expect_equal(find_random(Mc2),
                 list(random = "id"))
