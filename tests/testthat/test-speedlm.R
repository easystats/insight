if (require("testthat") && require("insight") && require("speedglm")) {
  ...
  m2 <- speedlm(log(mpg) ~ log(hp) + cyl + I(cyl^2) + poly(wt, degree = 2, raw = TRUE), data = mtcars)
  ...
  test_that("find_parameters", {
    expect_equal(
      find_parameters(m1),
      list(
        conditional = c("(Intercept)", "Petal.Width", "Speciesversicolor", "Speciesvirginica")
      )
    )
    expect_equal(
      find_parameters(m2),
      list(
        conditional = c("(Intercept)", "log(hp)", "cyl", "I(cyl^2)", "poly(wt, degree = 2, raw = TRUE)1", "poly(wt, degree = 2, raw = TRUE)2")
      )
    )
  ...
  test_that("find_terms", {
    ...
    expect_equal(
      ...
      list(
        response = "log(mpg)",
        conditional = c("log(hp)", "cyl", "I(cyl^2)", "poly(wt, degree = 2, raw = TRUE)")
      )
    )
  })
