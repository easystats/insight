...
test_that("get_predicted - lm", {
  ...
  expect_equal(
    mean(summary(rezlink)$CI_high - summary(rezrela)$CI_high),
    0,
    tolerance = 1e-3
  )
  ...
  expect_equal(
    mean(abs(as.data.frame(ref$fit)$fit - summary(rez)$Predicted)),
    0,
    tolerance = 0.1
  )
  expect_equal(
    mean(abs(as.data.frame(ref$fit)$lwr - summary(rez)$CI_low)),
    0,
    tolerance = 0.5
  )
...
test_that("get_predicted - glm", {
  ...
  ref <- as.data.frame(suppressWarnings(link_inverse(x)(predict.lm(
    x,
    interval = "confidence"
  ))))
  ...
  rez <- suppressWarnings(summary(get_predicted(
    x,
    iterations = 800,
    verbose = FALSE,
    ci = 0.95
...
test_that("get_predicted - lmerMod", {
  ...
  ref <- get_predicted(
    glmmTMB::glmmTMB(mpg ~ am + (1 | cyl), data = mtcars),
    predict = "expectation",
    ci = 0.95
  )
...
test_that("get_predicted - merMod", {
  ...
  expect_equal(
    max(abs(rezrela - stats::predict(x, type = "response"))),
    0,
    tolerance = 1e-4
  )
  ...
  expect_equal(
    mean(abs(as.data.frame(rezrela)$SE - as.data.frame(rez_ref)$SE)),
    0,
    tolerance = 0.2
  )
})
...
test_that("get_predicted - mgcv::gam and gamm", {
  ...
  rez2 <- get_predicted(
    x,
    data = data.frame(am = c(0, 0, 1), wt = c(2, 3, 4)),
    ci = 0.95,
  ...
  expect_equal(
    max(abs(rez - stats::predict(x$gam, type = "response"))),
    0,
    tolerance = 1e-4
  )
  expect_identical(nrow(as.data.frame(rez)), 32L)
...
test_that("get_predicted - rstanarm", {
  ...
  rezrela <- summary(get_predicted(x, predict = "expectation", ci = 0.95))
  rezpred <- summary(get_predicted(x, predict = "prediction", ci = 0.95))
  rezrela2 <- summary(get_predicted(
    x,
    predict = "expectation",
    ci = 0.95,
    include_random = FALSE
  ))
  rezpred2 <- summary(get_predicted(
    x,
    predict = "prediction",
    ci = 0.95,
    include_random = FALSE
  ...
  rezrela3 <- summary(
    get_predicted(x, predict = "expectation", ci = 0.95, data = mtcars["am"]),
    verbose = FALSE
  )
...
test_that("lm: get_predicted vs barebones `predict()`", {
  ...
  unknown3 <- suppressWarnings(as.data.frame(get_predicted(
    mod,
    ci = 0.95,
    predict = "response"
  )))
...
test_that("hurdle: get_predicted matches `predict()`", {
  ...
  unknown <- suppressWarnings(get_predicted(
    mod,
    predict = "zero",
    ci = 0.95,
    verbose = FALSE
...
test_that("bugfix: used to return all zeros", {
  ...
  expect_error(expect_warning(get_predicted(
    mod,
    predict = "original",
    ci = 0.95,
    verbose = FALSE
...
test_that("brms: `type` in ellipsis used to produce the wrong intervals", {
  ...
  expect_gt(
    max(
      x$Predicted - get_predicted(model, centrality_function = stats::median)$Predicted
    ),
    0
  )
