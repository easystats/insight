...
- `options(easystats_errors = <value>)` accepts `TRUE` or `FALSE`. If
  `TRUE`, functions that typically suppress errors (via
  [`tryCatch()`](https://rdrr.io/r/base/conditions.html)) will instead
  halt execution and throw an error when an issue occurs.
