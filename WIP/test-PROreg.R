...
test_that("get_random", {
  expect_identical(get_random(m1), dat["z"])
})
...
test_that("get_predictors", {
  expect_named(get_predictors(m1), "x")
})
...
test_that("get_data", {
  expect_identical(nrow(get_data(m1)), 100L)
  expect_named(get_data(m1), c("y", "x", "z"))
})
...
test_that("find_variables", {
  ...
  expect_identical(find_variables(m1, flatten = TRUE), c("y", "x", "z"))
})

test_that("n_obs", {
  expect_identical(n_obs(m1), 100L)
})
...
test_that("find_parameters", {
  ...
  expect_identical(get_parameters(m1)$Parameter, c("(Intercept)", "x"))
})
