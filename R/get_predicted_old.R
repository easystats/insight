...
#' @rdname get_predicted
#' @export
get_predicted.lm <- function(x, newdata = NULL, predict = "response", ci = 0.95, ci_type = "confidence", vcov_estimation = NULL, vcov_type = NULL, vcov_args = NULL, ...) {
...
get_predicted.glmmTMB <- function(x, newdata = NULL, ci = 0.95, transform = "response", include_random = TRUE, ...) {
  ...
  # CI
  ci_vals <- .get_predicted_se_to_ci_old(x, predicted = out, se = rez$se.fit, ci = ci)
...
#' @rdname get_predicted
#' @export
get_predicted.stanreg <- function(x, newdata = NULL, ci_type = "confidence", transform = "response", include_random = TRUE, include_smooth = TRUE, iterations = NULL, ...) {
  ...
  if (ci_type == "confidence") {
    if (transform == TRUE) {
      out <- rstantools::posterior_epred(x, newdata = args$newdata, re.form = args$re.form, draws = iterations, ...)
    } else {
      out <- rstantools::posterior_linpred(x, newdata = args$newdata, transform = FALSE, re.form = args$re.form, draws = iterations, ...)
    }
  } else {
    out <- rstantools::posterior_predict(x, newdata = args$newdata, transform = transform, re.form = args$re.form, draws = iterations, ...)
  }
...
.get_predicted_ci_analytic <- function(x, predicted, newdata, ci = 0.95, ci_type = "confidence", vcov_estimation = NULL, vcov_type = NULL, vcov_args = NULL, transform = NULL) {
  ...
  out <- .get_predicted_se_to_ci_old(x, predicted = predicted, se = se, ci = ci)
...
.get_predicted_se_to_ci_old <- function(x, predicted, se = NULL, ci = 0.95) {
  if (is.null(ci)) {
