...
.find_random_effects <- function(x, f, split_nested) {
  ...
  if (obj_has_name(f, "random")) {
    if (is.list(f$random)) {
      r1 <- unique(unlist(lapply(f$random, function(.x) get_model_random(.x, split_nested, inherits(x, c("MCMCglmm", "gee", "LORgee", "felm", "feis"))))))
    } else {
      r1 <- unique(unlist(get_model_random(f$random, split_nested, inherits(x, c("MCMCglmm", "gee", "LORgee", "felm", "feis")))))
    }
  } else {
    r1 <- NULL
  }
  ...
  if (obj_has_name(f, "zero_inflated_random")) {
    if (is.list(f$zero_inflated_random)) {
      r2 <- unique(unlist(lapply(f$zero_inflated_random, function(.x) get_model_random(.x, split_nested, inherits(x, c("MCMCglmm", "gee", "LORgee", "felm", "feis"))))))
    } else {
      r2 <- unique(get_model_random(f$zero_inflated_random, split_nested, inherits(x, c("MCMCglmm", "gee", "LORgee", "felm", "feis"))))
    }
  } else {
    r2 <- NULL
  }
