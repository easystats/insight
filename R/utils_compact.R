...
compact_list <- function(x, remove_na = FALSE) {
  if (remove_na) {
    x[!sapply(x, function(i) !is_model(i) && !inherits(i, c("Formula", "gFormula")) && !is.function(i) && (length(i) == 0L || is.null(i) || (length(i) == 1L && is.na(i)) || all(is.na(i)) || any(as.character(i) == "NULL", na.rm = TRUE)))]
  } else {
    x[!sapply(x, function(i) !is_model(i) && !inherits(i, c("Formula", "gFormula")) && !is.function(i) && (length(i) == 0L || is.null(i) || any(as.character(i) == "NULL", na.rm = TRUE)))]
  }
}
